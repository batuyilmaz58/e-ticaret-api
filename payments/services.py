...
def create_payment(user, order, card_data):
    ...
    total_items_price = sum(Decimal(item.price) * item.quantity for item in order.order_items.all())
    ...
    buyer = {
        'id': str(user.id),
        'name': user.first_name if user.first_name else 'Ad', 
        'surname': user.last_name if user.last_name else 'Soyad',
    ...
    billingAddress = {
        'contactName': order.billing_address.fullname if order.billing_address else "",
    ...
    deliveryAddress = {
        'contactName': order.delivery_address.fullname if order.delivery_address else "",
        'city': order.delivery_address.city.name if order.delivery_address and order.delivery_address.city else "",
        'country': 'Turkey',
